...
// "Silk" — 21st.dev Shader Builder flow shader. Plain WebGL1, no
// dependencies. Colours/uniforms are the exact values the client specified;
// see the fragment shader comment for the packed-uniform layout.
...
export function SilkShader({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Cursor is off per spec: presence stays 0 for the component's lifetime.
    ...
    const updateLayout = () => {
      ...
      if (reduceMotion) {
        // Static: one frame, no loop — motion is what reduced-motion opts out of.
        drawFrame(0);
      } else {
        requestRender();
      }
